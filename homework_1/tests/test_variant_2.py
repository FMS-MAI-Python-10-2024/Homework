...
class TestVariant2(unittest.TestCase):
    ...
    def generate_test_cases(self, solve_func, num_tests=40, input_range=(3, 100)):
        test_cases = []
    ...
    def generate_random_input(self, input_range):
        # Генерация размера матрицы (n от 3 до 100)
        n = random.randint(input_range[0], input_range[1])

        # Генерация матрицы n*n с элементами от 1 до 100
        matrix = [[random.randint(1, 100) for _ in range(n)] for _ in range(n)]

        # Преобразование матрицы в строковый формат для тестового ввода
    ...
    def run_tests_for_student(self, student_name, variant_number):
        ...
        if not Path(test_params_file).exists():
            ...
            test_cases = self.generate_test_cases(solve_func, num_tests=40, input_range=(3, 100))
